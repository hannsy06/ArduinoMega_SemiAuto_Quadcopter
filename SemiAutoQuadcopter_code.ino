...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
float pid_p_gain_roll = 1.4;               //default=1.3 Gain setting for the roll P-controller 
float pid_i_gain_roll = 0.06;              //default=0.04 Gain setting for the roll I-controller   
float pid_d_gain_roll = 13.8;              //default=18 Gain setting for the roll D-controller
...
void setup() {
  ...
  DDRH |= B01111000;                                                                    // (PORDH:PH3,PH4,PH5) 6,7,8
  //  gps_setup();
  testled();
  ...
  for (cal_int = 0; cal_int < 1250 ; cal_int ++) {                          //Wait 5 seconds before continuing.
    ...
    PORTB &= B00001111;                                                     //Set digital port 10, 11, 12 and 13 low.
  ...
  //  battery_voltage = ((float)analogRead(0) + 135) * 1.2317; // pin 5400mA
  //======================================================================
  ...
  for (start = 1; start <= 100; start++) {                       //This loop runs 100 times.
    ...
    delayMicroseconds(6400);                                                   //The main program loop also runs 250Hz (4ms per loop).
  }
   actual_pressure = 0;              
  //actual_pressure = 0;
...
void loop() { 
    ...
    if(receiver_input_channel_2 <=1300){
       ...
       { 
        ...
        if(debug_function == 1)        
        {
           ...
           add_roll   = data_buffer1.toFloat(); 
           add_pitch   = data_buffer2.toFloat();  
           correction_angle   = data_buffer3.toFloat();  
           throttle_altitude = data_buffer4.toInt();
  ...
  if (receiver_input_channel_6 >= 1350 ) flight_mode = 2;
  
  if (flight_mode == 2 && receiver_input_channel_5 >= 1300)flight_mode = 3;
  
  if (receiver_input_channel_6<=1200)flight_mode = 1;
...
//===============================================================
 ...
 {   //If the quadcopter is not in auto-level mode
  ...
  if (receiver_input_channel_3 < 1050 && receiver_input_channel_4 < 1060) 
  {
    start = 1;
    ground_pressure = actual_pressure;
  }
  //When yaw stick is back in the center position start the motors (step 2).
  // condition: actual_pressure-ground_pressure<=1 make sure that the pressure is stable
  if ((actual_pressure-ground_pressure<=1)&& start == 1 && receiver_input_channel_3 < 1050 && receiver_input_channel_4 > 1450) {
    start = 2;
  ...
  if (start == 2 && receiver_input_channel_3 < 1050 && receiver_input_channel_4 > 1940)start = 0;
